...
- v2.0.4 - CCSR TensorRT 自动化安装器与依赖栈更新：参考 SeedVR2 安装架构对 CCSR TensorRT 的安装流水线进行了全面升级与现代化。新增根目录下的一键批处理安装脚本 `Install TensorRT CCSR.bat` 与 PowerShell 自动化脚本 `scripts/install.ps1`，自动定位 ComfyUI 内置 Python 环境（`python_embeded`），检测 PyTorch / CUDA 运行状态，使用 `--no-deps` 安全安装依赖，拉取缺失的引擎并记录完整日志至 `outputs/install.log`。将所有 TensorRT 依赖统一整合至根目录 `requirements.txt`（`triton-windows==3.8.0.post28`、`tensorrt-rtx==1.6.1.120`、`onnx==1.22.0`、`onnxscript==0.7.1`、`polygraphy==0.53.4`），并在 `install.py` 中加入了旧版本自动升级机制。实现了引擎与辅助权重文件（`ccsr_apply_f16io.rtxplan` 与 `ccsr_trt_aux.safetensors`）在缺失时自动从 Hugging Face（`ussoewwin/CCSR-TensorRT-Engine`）流式下载并放置于 `nodes/CCSR/trt_engines/`。新增了用于全面诊断运行栈与引擎实体的 `scripts/verify_install.py`，为 `LoadCCSRModelTensorRT` 增加了运行时缺失防护与引导提示，并在中英文 README 中同步了完整的安装指南。（[发布说明](v2.0.4.md)）

- v2.0.3 - CCSR 仅保留 TensorRT：CCSR 的 PyTorch 节点（`CCSR_Upscale`、`CCSR_Model_Select`、`DownloadAndLoadCCSRModel`，含 ConvRot INT8 路径）已**移除** - 本插件现在只提供 **TensorRT** 路径（`LoadCCSRModelTensorRT`、`CCSR_Upscale_TRT`）。TRT 引擎运行 ControlNet+UNet 的 apply-model（约 1.4 倍于 fp16 PyTorch），使 fp16 / INT8 的 PyTorch 推理不再必要（更慢且更占显存）。引擎 + 辅助权重（VAE/cond_encoder）发布在 Hugging Face；`install.py` 会确保 TensorRT-RTX 运行时并指向下载地址。同时清除了 kijai 上游残留文件（`nodes/CCSR/.tracking`、嵌套的 `requirements.txt`、`pyproject.toml`、示例工作流），README 文档也已改为仅描述 TRT 路径。（[发布说明](v2.0.3.md)）

- v2.0.2 - CCSR ConvRot INT8 与 TensorRT 加速：CCSR 的 fp16 节点（`CCSR_Model_Select` / `DownloadAndLoadCCSRModel`）现在支持 **ConvRot INT8** 检查点 - UNet（ConvRot Linear）与 ControlNet（普通 INT8 Conv2d）被量化（3.2 GB → 约 2.0 GB 文件，VRAM 节省约 1.1 GiB；VAE / cond_encoder 保持 fp16），通过 `comfy_quant` 自动检测并以量化加载算子构建。新增 **TensorRT** 节点（`LoadCCSRModelTensorRT`、`CCSR_Upscale_TRT`）：仅引擎加载（`nodes/CCSR/trt_engines/` 中的引擎 + 引擎旁的 aux VAE/cond_encoder 权重，无需完整检查点），ControlNet+UNet 在 TensorRT 上运行（约 1.4 倍于 fp16 PyTorch）。`steps` 现在是实际扩散步数：保留 t_max/t_min 频段设计，同时加密调度，使截断后的范围恰好包含 `steps` 个时间步。（[发布说明](v2.0.2.md)）
