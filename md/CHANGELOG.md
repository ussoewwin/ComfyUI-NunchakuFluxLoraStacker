...
- v2.0.4 - CCSR TensorRT Automated Installer & Stack Modernization: Overhauled the CCSR TensorRT installation pipeline following the SeedVR2 installer pattern. Added root one-click installer `Install TensorRT CCSR.bat` and PowerShell automation `scripts/install.ps1` that automatically locate the ComfyUI embedded Python environment, probe PyTorch / CUDA capabilities, install dependencies safely with `--no-deps`, download missing engine artifacts, and log execution to `outputs/install.log`. Modernized runtime stack with all dependencies unified in `requirements.txt` (`triton-windows==3.8.0.post28`, `tensorrt-rtx==1.6.1.120`, `onnx==1.22.0`, `onnxscript==0.7.1`, `polygraphy==0.53.4`) and added auto-upgrade detection for outdated Triton environments in `install.py`. Integrated automatic streaming download for missing engine artifacts (`ccsr_apply_f16io.rtxplan` and `ccsr_trt_aux.safetensors`) from Hugging Face (`ussoewwin/CCSR-TensorRT-Engine`) into `nodes/CCSR/trt_engines/`. Added `scripts/verify_install.py` for comprehensive runtime and engine validation, added runtime availability checks to `LoadCCSRModelTensorRT`, and updated documentation across both English and Chinese READMEs. ([Release Notes](https://github.com/ussoewwin/ComfyUI-NunchakuFluxLoraStacker/releases/tag/v2.0.4))

- v2.0.3 - CCSR TensorRT-only: The CCSR PyTorch nodes (`CCSR_Upscale`, `CCSR_Model_Select`, `DownloadAndLoadCCSRModel`, incl. the ConvRot INT8 path) were **removed** - the pack now ships the **TensorRT-only** CCSR path (`LoadCCSRModelTensorRT`, `CCSR_Upscale_TRT`). The TRT engine runs the ControlNet+UNet apply-model (~1.4x vs fp16 PyTorch), which made fp16 / INT8 PyTorch inference obsolete (slower and heavier). Engine + aux weights (VAE/cond_encoder) are published on Hugging Face; `install.py` ensures the TensorRT-RTX runtime stack and points at the download. kijai upstream leftovers (`nodes/CCSR/.tracking`, nested `requirements.txt`, `pyproject.toml`, example workflow) were cleaned out, and README docs now describe the TRT-only path. ([Release Notes](https://github.com/ussoewwin/ComfyUI-NunchakuFluxLoraStacker/releases/tag/v2.0.3))

- v2.0.2 - CCSR ConvRot INT8 + TensorRT: The CCSR fp16 nodes (`CCSR_Model_Select` / `DownloadAndLoadCCSRModel`) now accept **ConvRot INT8** checkpoints - the UNet (ConvRot Linear) and ControlNet (plain INT8 Conv2d) are quantized (3.2 GB → ~2.0 GB file, ~1.1 GiB VRAM saved; VAE / cond_encoder stay fp16) via automatic `comfy_quant` detection with quantized-loading ops. Added **TensorRT** nodes (`LoadCCSRModelTensorRT`, `CCSR_Upscale_TRT`): engine-only load from `nodes/CCSR/trt_engines/` (aux VAE/cond_encoder weights beside the engine, no full checkpoint required), ControlNet+UNet run on TensorRT (~1.4x vs fp16 PyTorch). `steps` is now the effective diffusion step count: the t_max/t_min band design is preserved while the schedule is densified so the truncated range holds exactly `steps` timesteps. ([Release Notes](https://github.com/ussoewwin/ComfyUI-NunchakuFluxLoraStacker/releases/tag/v2.0.2))

- v2.0.1 — Model Patch Loader ConvRot INT8 for QwenImage: **Model Patch Loader** (`ModelPatchLoaderCustom`) now applies the ConvRot INT8 path (comfy_quant detection → `mixed_precision_ops` + BF16 graph) to **all** model-patch types, not just Z-Image. Loading an INT8 `QwenImageBlockWiseControlNet` (e.g. `qwen_image_canny_diffsynth_controlnet_convrot_int8.safetensors`) previously built the graph with `manual_cast` + `weight_dtype()=int8`, which loaded the raw INT8 weights as plain Parameters and produced **garbage / noise against Nunchaku Qwen Image** models. Weights now stay **INT8 in VRAM** (verified: 1136 MB vs 2266 MB BF16, ~50% saving) and forward runs on the comfy-kitchen `int8_linear` kernel with online ConvRot rotation, matching the HSWQ model patch loader. ([Release Notes](https://github.com/ussoewwin/ComfyUI-NunchakuFluxLoraStacker/releases/tag/v2.0.1))

- v2.0.0 — Model Patch Loader ConvRot INT8: **Model Patch Loader** (`ModelPatchLoaderCustom`) now loads comfy-native **ConvRot INT8** Z-Image ControlNet checkpoints (e.g. `Z-Image-Turbo-Fun-Controlnet-Union-2.1-lite-2601-8steps_convrot_int8.safetensors`). INT8 is detected automatically via `comfy_quant` metadata, the module graph is built in BF16 and quantized weights are attached by `mixed_precision_ops`, so weights stay **INT8 in memory** and inference runs on the comfy-kitchen `int8_linear` kernel with online ConvRot activation rotation. Works with both GPU loading and CPU offload; BF16 checkpoints keep the existing path. Fixes the `Only Tensors of floating point and complex dtype can require gradients` crash. ([Release Notes](https://github.com/ussoewwin/ComfyUI-NunchakuFluxLoraStacker/releases/tag/v2.0.0))
